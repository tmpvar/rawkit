...
void ps_uv_write_cb(uv_write_t* req, int status);

void ps_uv_write(ps_duplex_io_t *duplex, ps_val_t *value);

ps_val_t *ps_uv_sink_fn(ps_t *sink, ps_stream_status status);
ps_val_t *ps_uv_source_fn(ps_t *source, ps_stream_status status);
